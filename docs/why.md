...
`go-ruby-rouge/rouge` reimplements Ruby's `rouge` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-rouge/rouge` directly, with no Ruby runtime;
